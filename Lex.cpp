...
//checking operator
bool Lex::theOperator(const char x) const
{
	int operators[8] = { '+', '-', '/', '*', '<','>',':', '=' };
	for (int i = 0; i < 8; i++) {
		if (operators[i] == x) {
...
//checking separator
bool Lex::theSeparator(const char x) const
{
	int separator[10] = { '(',')', '{', '}', '%', '@', ',', '[', ']', ';' };
	for (int i = 0; i < 10; i++) {
		if (separator[i] == x) {
...
//checking keyword
bool Lex::theKeyword(string identifier) const
{
...
int Lex::char_to_col(const char x) const
{
	if (isdigit(x))
		return 1;
	else if (input == '.')
...
void Lex::lexer(ifstream& file)
{
	...
	else if (classify == 1)
	{
		...
		if (theOperator(str[0]) || str == ":=" || str == "/=" || str == "<=" || str == ">=")
		{
